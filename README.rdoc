...
* Sidekiq for fast background jobs
  * Emails from User and Devise are enqueued before sending
* [BEM CSS](http://bem.info/method/) – makes it easier to search and replace and maintain larger CSS projects


## Best Practices

Respond to requests ASAP
* Never do anything in the foreground that can be done by a worker
* Emails are always enqueued before sending
* Ideally, POST, PUT, and DELETE are enqueued — not currently implemented
...
1. Replace `StarterKit` with your app brand name (the name users will see) in...
	1. config/locales/en.yml
1. Replace `starterkit` with your namespace friendly app name in...
  1. config/application.yml
1. Replace `StarterKit` with your camelcased app name in...
...
Running `rails server` will start WEBrick. To run unicorn in development, use `unicorn -c config/unicorn.rb`.
...
* [http://localhost:3000/rails/mailers](http://localhost:3000/rails/mailers) – Rails 4.1 built-in method
* [http://0.0.0.0:3000/p/email?premail=true&layout=email](http://0.0.0.0:3000/p/email?premail=true&layout=email) – StarterKit method with query param configs
